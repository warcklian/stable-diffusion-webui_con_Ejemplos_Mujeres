#!/usr/bin/env python3
"""
Script para probar proporciones optimizadas para Realistic Vision V6.0
"""
...
def generar_prompt_realistic_vision_v6(perfil, edad=25):
    """Genera prompt optimizado para Realistic Vision V6.0"""
...
def test_realistic_vision_v6():
    """Prueba proporciones optimizadas para Realistic Vision V6.0"""
    print("📸 PROBANDO PROPORCIONES PARA REALISTIC VISION V6.0")
    print("=" * 60)
    ...
    for i in range(3):
        ...
        # Generar prompt
        prompt, negative_prompt = generar_prompt_realistic_vision_v6(perfil, 25)
        ...
        # Analizar proporciones optimizadas para Realistic Vision V6.0
        rv6_indicators = [
            "head positioned in upper third of frame", "head in upper portion of image",
            "head not touching top edge", "small space above head", "minimal clearance above head",
            "head well below top border", "head centered vertically in upper third",
            "face fills 70% of image height", "face occupies most of frame",
            "head takes up most of image", "large head in frame", "head dominates the image",
            "head and shoulders fill frame", "shoulders at bottom of image",
            "shoulders visible at bottom edge", "head and shoulders composition",
            "passport photo proportions", "ID photo proportions", "document photo proportions",
            "official photo proportions", "government photo proportions", "passport style proportions",
            "ID card proportions", "document proportions", "official document proportions",
            "government document proportions", "tight headshot framing", "close-up headshot",
            "head fills most of frame", "face centered in upper portion",
            "head positioned in upper 60% of image", "eyes positioned at 45% from top",
            "head and shoulders visible", "shoulders at bottom edge", "head not touching top",
            "head not touching sides", "proper head size for passport", "correct head proportions",
            "head size appropriate for ID", "head size suitable for document",
            "head size perfect for passport", "head size ideal for ID card",
            "head size optimal for document", "head size correct for official photo",
            "head size proper for government ID", "proper head size", "correct head size",
            "head not too large", "head not too small", "head proportional", "head well proportioned",
        ...
        rv6_count = sum(1 for indicator in rv6_indicators if indicator in prompt)
        print(f"\n   🎯 INDICADORES RV6: {rv6_count}/{len(rv6_indicators)}")
        
        # Analizar negativos optimizados para Realistic Vision V6.0
        rv6_negative_indicators = [
            "head too high in frame", "head at top of image", "head touching top edge",
            "head filling top of frame", "head too small in frame", "head too far from camera",
            "head not filling frame", "head not dominating image", "face too small",
            "face not filling frame", "head not centered vertically", "head not in upper third",
            "head positioned too low", "head in middle of frame", "head in lower portion",
            "head not in upper portion", "shoulders not visible", "shoulders cut off",
            "shoulders missing", "head only visible", "no shoulders", "shoulders not at bottom",
            "wrong proportions", "incorrect proportions", "bad proportions", "poor proportions",
            "wrong composition", "incorrect composition", "bad composition", "poor composition",
            "head too large for frame", "head too small for frame", "head not appropriate size",
            "head size incorrect for passport", "head size wrong for ID", "head size inappropriate for document",
            "head size not suitable for passport", "head size not ideal for ID card",
            "head size not optimal for document", "head size not correct for official photo",
            "head size not proper for government ID", "head touching top edge", "head touching sides",
            "head touching bottom", "head filling entire frame", "head too close to edges",
            "head too close to top", "head too close to sides", "head too close to bottom",
            "insufficient space around head", "no space around head", "head filling frame completely",
            "head dominating entire image", "head taking up whole frame"
        ]
        
        rv6_negative_count = sum(1 for indicator in rv6_negative_indicators if indicator in negative_prompt)
        print(f"   🚫 INDICADORES NEGATIVOS RV6: {rv6_negative_count}/{len(rv6_negative_indicators)}")
        
        # Calcular score total
        total_score = (rv6_count + rv6_negative_count) / (len(rv6_indicators) + len(rv6_negative_indicators)) * 100
        
        print(f"\n   📊 SCORE TOTAL RV6: {total_score:.1f}%")
        
        if total_score >= 90:
            print("   ✅ EXCELENTE - Optimizado para Realistic Vision V6.0")
        elif total_score >= 80:
            print("   ⚠️ BUENO - Mayormente optimizado para RV6")
        elif total_score >= 70:
            print("   ⚠️ REGULAR - Necesita mejoras para RV6")
        else:
            print("   ❌ MALO - No optimizado para RV6")
    
    print(f"\n\n📊 RESUMEN DE OPTIMIZACIÓN REALISTIC VISION V6.0")
    print("=" * 50)
    print("✅ Prompts optimizados para Realistic Vision V6.0")
    print("✅ Proporciones específicas para fotografía realista")
    print("✅ Términos que RV6 entiende perfectamente")
    print("✅ Composición visual correcta para pasaportes")
    print("✅ Prompt negativo reforzado contra errores de proporción")
    
    print(f"\n🎯 ESTRATEGIA REALISTIC VISION V6.0 IMPLEMENTADA:")
    print("   - 'head positioned in upper third of frame' (composición visual)")
    print("   - 'face fills 70% of image height' (proporción específica)")
    print("   - 'head dominates the image' (tamaño correcto)")
    print("   - 'passport photo proportions' (concepto fotográfico)")
    print("   - 'head size perfect for passport' (especificación clara)")
    print("   - 'tight headshot framing' (técnica fotográfica)")
    print("   - ¡Optimizado para Realistic Vision V6.0!")

if __name__ == "__main__":
    print("🚀 INICIANDO PRUEBAS DE REALISTIC VISION V6.0")
    print("=" * 60)
    
    test_realistic_vision_v6()
