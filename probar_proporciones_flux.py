#!/usr/bin/env python3
"""
Script para probar proporciones compatibles con modelos FLUX
"""
...
def generar_prompt_flux_compatible(perfil, edad=25):
    """Genera prompt compatible con modelos FLUX para proporciones correctas"""
...
def test_flux_compatible_proportions():
    ...
    for i in range(3):
        ...
        # Generar prompt
        prompt, negative_prompt = generar_prompt_flux_compatible(perfil, 25)
        ...
        # Analizar proporciones FLUX-compatibles
        flux_indicators = [
            ...
            "government document proportions", "proper head size", "correct head size",
        ...
        flux_count = sum(1 for indicator in flux_indicators if indicator in prompt)
        print(f"\n   🎯 INDICADORES FLUX: {flux_count}/{len(flux_indicators)}")
        
        # Analizar negativos FLUX-compatibles
        flux_negative_indicators = [
        ...
        flux_negative_count = sum(1 for indicator in flux_negative_indicators if indicator in negative_prompt)
        print(f"   🚫 INDICADORES NEGATIVOS FLUX: {flux_negative_count}/{len(flux_negative_indicators)}")
        
        # Calcular score total
        total_score = (flux_count + flux_negative_count) / (len(flux_indicators) + len(flux_negative_indicators)) * 100
        
        print(f"\n   📊 SCORE TOTAL FLUX: {total_score:.1f}%")
        
        if total_score >= 90:
            print("   ✅ EXCELENTE - Compatible con modelos FLUX")
        elif total_score >= 80:
            print("   ⚠️ BUENO - Mayormente compatible con FLUX")
        elif total_score >= 70:
            print("   ⚠️ REGULAR - Necesita mejoras para FLUX")
        else:
            print("   ❌ MALO - No compatible con FLUX")
    
    print(f"\n\n📊 RESUMEN DE COMPATIBILIDAD FLUX")
    print("=" * 50)
    print("✅ Prompts optimizados para modelos FLUX")
    print("✅ Proporciones visuales en lugar de medidas exactas")
    print("✅ Términos que FLUX puede entender y aplicar")
    print("✅ Composición visual correcta para pasaportes")
    print("✅ Prompt negativo reforzado contra errores de proporción")
    
    print(f"\n🎯 ESTRATEGIA FLUX IMPLEMENTADA:")
    print("   - 'head positioned in upper third of frame' (en lugar de 2-5mm)")
    print("   - 'face fills 70% of image height' (en lugar de 30-34mm)")
    print("   - 'head dominates the image' (en lugar de medidas exactas)")
    print("   - 'passport photo proportions' (concepto visual)")
    print("   - 'head and shoulders composition' (composición clara)")
    print("   - ¡Compatible con modelos FLUX para mejores resultados!")

if __name__ == "__main__":
    print("🚀 INICIANDO PRUEBAS DE COMPATIBILIDAD FLUX")
    print("=" * 60)
    
    test_flux_compatible_proportions()
