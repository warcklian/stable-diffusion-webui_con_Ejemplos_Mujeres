#!/usr/bin/env python3
"""
Script para probar los estándares exactos del SAIME Venezuela
"""
...
def generar_prompt_saime_venezuela(perfil, edad=25):
    """Genera prompt con estándares exactos del SAIME Venezuela"""
    ...
    prompt_parts = [
        ...
        "2-5mm space above head",
        "minimal space above head",
        "head 2-5mm from top edge",
        "head positioned 2-5mm from top",
        "face height 30-34mm",
        "face height 30-34 millimeters",
        "head height 30-34mm",
        "head height 30-34 millimeters",
        "10-15mm space each side of head",
        "10-15mm space left and right of head",
        "head 10-15mm from left edge",
        "head 10-15mm from right edge",
        "20-25mm space below chin",
        "20-25mm space under chin",
        "chin 20-25mm from bottom edge",
        "chin positioned 20-25mm from bottom",
        "35mm x 45mm photo dimensions",
...
def test_saime_standards():
    ...
    for i in range(3):
        ...
        # Generar prompt
        prompt, negative_prompt = generar_prompt_saime_venezuela(perfil, 25)
        ...
        # Analizar estándares SAIME específicos
        saime_indicators = [
            "SAIME Venezuela passport photo", "official SAIME specifications", "Venezuelan passport requirements",
            ...
            "head centered in frame", "shoulders at bottom edge", "2-5mm space above head",
            "minimal space above head", "head 2-5mm from top edge", "head positioned 2-5mm from top",
            "face height 30-34mm", "face height 30-34 millimeters", "head height 30-34mm",
            "head height 30-34 millimeters", "10-15mm space each side of head",
            "10-15mm space left and right of head", "head 10-15mm from left edge",
            "head 10-15mm from right edge", "20-25mm space below chin",
            "20-25mm space under chin", "chin 20-25mm from bottom edge",
            "chin positioned 20-25mm from bottom", "35mm x 45mm photo dimensions",
            "35mm width 45mm height", "600 DPI resolution", "high resolution 600 DPI",
        ...
        saime_count = sum(1 for indicator in saime_indicators if indicator in prompt)
        print(f"\n   🎯 INDICADORES SAIME: {saime_count}/{len(saime_indicators)}")
        
        # Analizar negativos SAIME específicos
        saime_negative_indicators = [
            ...
            "too little space above head", "head filling frame", "head filling top",
            "head filling edges", "head filling sides", "head filling bottom",
            "head at top of image", "head near top edge", "head close to top",
            "head touching top border", "head touching top margin", "head too high in frame",
            "head positioned too high", "head not centered vertically", "head not in upper third",
            "head not in upper portion", "too much space above head", "excessive space above head",
            "head too far from top", "head positioned too low", "face height not 30-34mm",
            "face height incorrect", "head height not 30-34mm", "head height incorrect",
            "insufficient space at sides", "head too close to edges", "head touching left edge",
            "head touching right edge", "insufficient space below chin", "chin too close to bottom",
            "chin touching bottom edge", "wrong photo dimensions", "not 35mm x 45mm",
            "incorrect photo size", "low resolution", "not 600 DPI"
        ]
        
        saime_negative_count = sum(1 for indicator in saime_negative_indicators if indicator in negative_prompt)
        print(f"   🚫 INDICADORES NEGATIVOS SAIME: {saime_negative_count}/{len(saime_negative_indicators)}")
        
        # Calcular score total
        total_score = (saime_count + saime_negative_count) / (len(saime_indicators) + len(saime_negative_indicators)) * 100
        
        print(f"\n   📊 SCORE TOTAL SAIME: {total_score:.1f}%")
        
        if total_score >= 90:
            print("   ✅ EXCELENTE - Cumple todos los estándares SAIME")
        elif total_score >= 80:
            print("   ⚠️ BUENO - Cumple la mayoría de estándares SAIME")
        elif total_score >= 70:
            print("   ⚠️ REGULAR - Necesita mejoras en algunos estándares SAIME")
        else:
            print("   ❌ MALO - No cumple los estándares SAIME")
    
    print(f"\n\n📊 RESUMEN DE ESTÁNDARES SAIME VENEZUELA")
    print("=" * 50)
    print("✅ Prompts optimizados para estándares exactos del SAIME")
    print("✅ Medidas específicas implementadas según SAIME")
    print("✅ Control exacto de dimensiones y espaciado")
    print("✅ Especificaciones técnicas del SAIME aplicadas")
    print("✅ Prompt negativo reforzado contra errores SAIME")
    
    print(f"\n🎯 ESTÁNDARES SAIME IMPLEMENTADOS:")
    print("   - Tamaño: 35mm × 45mm (ancho × alto)")
    print("   - Espacio superior: 2-5mm desde cabeza al borde")
    print("   - Altura del rostro: 30-34mm")
    print("   - Espacio lateral: 10-15mm cada lado")
    print("   - Espacio inferior: 20-25mm desde barbilla")
    print("   - Resolución: 600 DPI")
    print("   - Fondo: Blanco liso sin texturas")
    print("   - ¡Listo para generar fotos según SAIME Venezuela!")

if __name__ == "__main__":
    print("🚀 INICIANDO PRUEBAS DE ESTÁNDARES SAIME VENEZUELA")
    print("=" * 60)
    
    test_saime_standards()
